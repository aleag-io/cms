...
<!-- BEGIN:fleet-baseline v1 -->
<!-- Generated by fleet/scripts/sync-agents.sh — do not edit inside these markers. -->

## Fleet conventions

These rules hold in every `aleag-io` repository. Anything written elsewhere in
this file is specific to this repo and takes precedence over this section.

- **Conventional Commits.** `feat:`, `fix:`, `docs:`, `refactor:`, `test:`,
  `chore:`; `BREAKING CHANGE:` in the footer. Never `WIP`, `update stuff`, or
  `fixed bug`. `things` and `waivers` cut releases from these messages with
  semantic-release, so the type you choose moves a version number.
- **Never commit to the production branch directly.** Always branch, then open a
  PR. In most repos the base is `main` and branches are `feat/<desc>` or
  `fix/<desc>` — but check this repo's own workflow section first. `cms`
  integrates through `preview` and treats `main` as production, so its feature
  branches come off `preview`, not `main`.
- **Never commit secrets.** `.env*` files are gitignored and stay that way. Keys
  belong in the deploy target's environment settings, not the repo.
- **`NEXT_PUBLIC_*` is public.** It is inlined into the client bundle. A secret
  in one is a published secret.
- **TypeScript strict** wherever TypeScript is used, with `@/*` aliased to the
  repo (or app) root.
- **Repos are not interchangeable.** Framework and library versions differ
  across the fleet — Next.js spans 14 to 16, Prisma spans 6 to 7, and the
  shadcn style and icon set differ per repo. Before copying a component, config
...
- **Never write a plan for one repo into another**, and never into the
  `fleet` meta-repo — plans belong with the code they describe. `fleet`
...
- **App Router only.** Server Components by default; `"use client"` only for
  hooks, interactive primitives, or browser APIs. Keep client components small
  and pass Server Components through as `children`.
- **Client components never import Prisma** or read server-only environment
  variables.
- **shadcn/ui lives in `components/ui/*`** — do not relocate it, `components.json`
  depends on the path. Compose with CVA variants and `cn()` from `lib/utils.ts`.
- **Use design tokens** (`bg-background`, `text-foreground`), never hardcoded
  colors. Tailwind is configured with CSS-variable tokens in `app/globals.css`.
- **Prisma client is a singleton** (`lib/prisma.ts`) so dev hot-reload does not
  exhaust connections. Use `prisma.$transaction` for multi-step atomic work.
- **Supabase Auth is the session source.** Row Level Security is the security
  boundary — the UI is not. Any table reachable through PostgREST needs policies
  before it holds real data.
- **Validate inputs with Zod** inside Server Actions, and revalidate with
  `revalidatePath` / `revalidateTag` after mutations.
<!-- END:fleet-profile-nextjs-supabase -->
