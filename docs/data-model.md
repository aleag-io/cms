...
> **Note:** Spouse relationships _within_ the same family unit are captured via `FamilyMember.relationship = 'spouse'`. `MemberRelationship` is for **cross-family** links — e.g., a member's parents who have their own separate family/member number at the same parish.
